...
describe("SummonerMatches Endpoint", () => {
	...
	it.skip("Expect a matchList to be 0", async () => {
		...
		const response: Response = await request(startedApp).get(`/api/data/summoner/forevermates`);
	...
	it.skip("Expect matches to update", async () => {
		...
		const response: Response = await request(startedApp).get(`/api/refresh/match/${summonerId}`);
